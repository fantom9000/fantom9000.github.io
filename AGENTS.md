...
| Ширина | `.project-sidebar p`, `.side-work__text p` | Логика |
|---|---|---|
| Десктоп (>960) | `max-width: 294px` | базовая колонка sidebar 22% |
| Планшет (480–960) | `max-width: 70%` | компактная колонка, удобная для чтения |
| Телефон (≤480) | `max-width: none` | вся доступная ширина — иначе текст слишком сжат |

...
- GitHub remote is connected: `https://github.com/fantom9000/fantom9000.github.io` (репозиторий переименован 2026-05-20 для user-page деплоя).
- **Live URL: https://fantom9000.github.io/** (задеплоено 2026-05-20 через GitHub Actions). Любой push в `main` запускает workflow `.github/workflows/deploy.yml` (~1 минута до зелёной галки → сайт обновлён).
- Current branch is `main`; all work is merged here. **Always work from `main` — do not continue old worktree branches.**
- Latest known commit (на момент этой записи): `1420da4 typograf.mjs: prefix unused tag param with underscore`. Реальный последний — `git log --oneline -5`.
- Astro check: **0 errors, 0 warnings, 0 hints** (с 2026-05-20).
...
- Welcome section uses 5 single-image phone screens (1093×2223 after alpha trim, lossless WebP since 2026-05-20 — раньше были PNG, конвертировали по результатам Lighthouse-аудита), simple grid, gap 10px, max-width 1385px. Mobile: horizontal scroll at 760px.
...
- `/projects/peptidy/` — fully re-exported and approved (2026-05-20): 4 PNG ×2 from Figma → lossless WebP, ~1.6 MB total. Old `main-screen.png` in project root removed — main-screen now lives in `figma-sections/` (как и в Бюросфере).
...
## Deploy (GitHub Pages, настроено 2026-05-20)

- **Live URL:** https://fantom9000.github.io/
- **Repo:** `https://github.com/fantom9000/fantom9000.github.io` (user-page репо, имя совпадает с username — поэтому root URL без подпапки)
- **History:** репо когда-то назывался `roman-portfolio`, потом был переименован в `portfolio`, потом в `fantom9000.github.io` (последнее имя освобождено путём переименования старой учебной странички в `study-page`).
- **Workflow:** `.github/workflows/deploy.yml` — стандартный Astro→Pages template. Запускается на push в `main` или вручную (workflow_dispatch). Один job (`build`) собирает сайт, второй (`deploy`) публикует через `actions/deploy-pages@v4`.
- **GitHub Settings → Pages → Source** должен быть выставлен в **GitHub Actions** (не «Deploy from a branch»). Один раз руками через веб-UI.
- **Поучительные истории CI** (для будущих агентов чтобы не повторили):
  - **pnpm-workspace.yaml требует поле `packages` для pnpm 9+.** Без него pnpm в CI падает с `ERROR packages field missing or empty`. У нас этот файл нужен ради `allowBuilds: { esbuild, sharp }`, поэтому `packages: ['.']` объявляет одно-пакетный «workspace».
  - **pnpm 11 требует Node ≥22.13.** На Node 20 падает с `ERR_UNKNOWN_BUILTIN_MODULE: node:sqlite`. Локально у Романа pnpm 11, поэтому в CI Node 22.
  - **Lockfile `overrides` (rollup wasm) валидируется строго.** Если меняешь версию pnpm — пересобрать lockfile, иначе `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH` на `--frozen-lockfile`.
  - **`cache: pnpm` в actions/setup-node** работает только если `pnpm-workspace.yaml` валидный (см. выше) — иначе фейлится на этапе Setup Node.
- **Git credentials:** на машине Романа в `.tools/gh/bin/gh` лежит локальный GitHub CLI, прописанный как credential helper для github.com (переопределяет osxkeychain). Push требует scope `workflow` для записи файлов в `.github/workflows/`. Если push отбит с «refusing to allow an OAuth App» — `./.tools/gh/bin/gh auth refresh -s workflow` добавляет недостающий scope.
- **Custom domain (опционально, потом):** добавить файл `public/CNAME` с содержимым домена, прописать DNS-записи у регистратора (4 A-записи на IP GitHub Pages + AAAA + CNAME на www), в Settings → Pages → Custom domain вписать домен. HTTPS GitHub Pages раздаёт сам через Let's Encrypt. Не поддерживается на `.рф` (нужен ASCII-домен).
...
- `.contact-links { flex-direction: row, gap: clamp(10px, 1.5vw, 20px) }` — горизонтально (на десктопе 20px, плавно ужимается до 10px на ≤666px). Сделано 2026-05-20, чтобы на ширинах 320-350 контакты не «налипали» на имя — закон близости требует gap внутри группы < gap между группами.
...
## Поучительные истории сессии 2026-05-20 (вторая половина, после деплоя)

- **iOS Safari в портретном режиме автоматически раздувает текст**, который считает «слишком мелким для смартфона». Это **не баг твоего кода**, это feature браузера. Видно как «Дизайнер интерфейсов» становится крупнее проектной спецификации только в портрете на iPhone. Фикс — добавить в `html { -webkit-text-size-adjust: 100%; text-size-adjust: 100%; }`. Стандартная настройка любого современного сайта. Должна быть с самого начала.
- **`label-content-name-mismatch` — реальное a11y-правило WCAG 2.5.3.** Если у `<a>` стоит `aria-label="X"`, а внутри видимый текст «Y» — это ломает voice-control: юзер говорит «click on Y», система ищет элемент с accessible name «Y», не находит. Скрин-ридер тоже произносит «X» вместо «Y». Правило: либо убирать `aria-label` (visible text становится accessible name автоматически), либо включать visible text в aria-label. У нас был `aria-label="На главную"` на `<a class="site-name">Роман Жалялов</a>` — убрали aria-label.
- **GitHub Pages не контролирует Cache-Control** для статичных ассетов — Lighthouse ругается на «inefficient cache lifetimes», но мы это исправить не можем. Структурное ограничение, фиксится только переездом на CDN (Cloudflare Pages, custom domain через Cloudflare proxy и т.п.). Сейчас не трогаем.
- **Lighthouse Performance измеряет LCP (above-the-fold), а не общий вес страницы.** Конвертация Welcome PNG → WebP сэкономила 6.2 МБ общего трафика (важно на мобильной сети!), но не сдвинула Performance score, потому что Welcome — это `loading="lazy"` ниже fold. Чтобы реально сдвинуть Performance, нужно работать с **первой картинкой выше fold** (у нас это превью Бюросферы с `loading="eager"`). Lighthouse ≠ user experience, у них разные метрики.
- **Невозможно полностью убрать визуальную подгрузку картинок на медленной сети, даже у топовых editorial-сайтов.** У Erno (Webflow, CDN, видео-постеры) тоже есть пролаги. Это физика интернета, не дефект. Делаются три вещи: (а) уменьшить вес картинок, (б) маскировать загрузку (LQIP / blur placeholder / video poster), (в) приоритизировать важные картинки (`fetchpriority="high"` на above-the-fold). Полностью «без подгрузки» не бывает.
- **Erno использует `<video autoplay loop muted>` с `poster` JPG как замену картинкам.** Постер показывается мгновенно, видео загружается фоном. Для статичных UI-портфолио (как у нас) эквивалент — LQIP. Roman пока не решил делать LQIP — в backlog.
- **Не описывать визуал чужих сайтов без реального осмотра.** Был эпизод: прочитал HTML erno.works, увидел в `sizes` атрибуте маленькие размеры (58-74px), сделал вывод «крошечные кружочки». На деле эти `sizes` были у **скрытого** `w-condition-invisible` img-элемента из Webflow CMS — видимый контент это `<video>` шириной 570-1150px. Урок: если описываешь как **выглядит** что-то — открой и посмотри, не выводи из разметки.
- **CI-зависимости (GitHub Actions) надо выверить под локалку:** `pnpm-workspace.yaml` требует `packages` field для pnpm 9+, pnpm 11 требует Node ≥22.13, `--frozen-lockfile` валидирует hash секции `overrides`. Lessons зафиксированы в `## Deploy` секции — не повторять.
...
- **Неразрывный пробел между парными именами брендов.** Двухсловные названия (например «Бюро Горбунова», «Mary Trufel», «Welcome Доставка») не должны разрываться по строке — это считается грубой типографической ошибкой. Использовать **U+00A0** (NBSP) между словами в исходнике. Применять выборочно — массовое склеивание создаёт «островки» неразрывного текста, которые ломают плотность на узких ширинах (см. инцидент 2026-05-20 с лидом главной). Типограф автоматически ставит NBSP только после коротких предлогов/союзов, парные имена нужно склеивать вручную. Текущий пример: `Бюро Горбунова` в `src/pages/index.astro:16`.
- **Семантика кликабельных зон.** Принцип: кликабельная зона = ровно то, что выглядит интерактивно. Никаких «фантомных» зон (пустое поле внутри `<a>`, sidebar-обёртка целиком как ссылка). Конкретно:
  - **Превью проектов на главной** (`ProjectPreview.astro`): `<article>` — обычный контейнер, **не** ссылка. Две независимые `<a>`: одна оборачивает картинку (`.project-visual`), другая оборачивает текст h2 (`<h2><a>...</a></h2>`). Описание `<p>`, номер, пустое поле — **не кликабельны**. Hover на любую из двух ссылок подсвечивает заголовок (через `:has()`-селектор).
  - **Имя сайта в хедере и футере** (`.site-name`): `<a href="/">` в обоих местах. `width: fit-content` обязательно — иначе block-элемент в grid-ячейке съедает всю ширину ячейки как кликабельную зону. Сделано 2026-05-20 после инцидента с фантомной зоной справа от имени.
  - **Анти-паттерн:** оборачивать sidebar-блок или article целиком в `<a>` ради «whole-card click». Это school A (Netflix/Pinterest) — оправдана для каталогов из 50+ карточек, но не для editorial-портфолио. У нас school B: predictability > generous touch target.
- **Hover-эффекты — иерархия по роли.** Принцип: разные роли ссылок → разные hover-фидбэки. Не путать с «несогласованностью».
  - **Имя сайта** (`.site-name`) — `opacity: 0.6` с `transition: opacity 0.2s ease`. **Без подчёркивания.** Логика: на главной клик по имени = no-op (перезагрузка), подчёркивание ложно обещает «уйдёшь на другую страницу». Opacity = «я кликабельно, но не обещаю переход».
  - **Контакты в хедере/футере** (`.contact-links a`) и **заголовки проектов** (`.project-preview h2`) — подчёркивание на hover. Эти ссылки **всегда** ведут на другую страницу (внешний ресурс или кейс), подчёркивание здесь честное.
  - **Параметры подчёркивания:** `text-decoration-thickness: 0.5px` (тонкий волосок), `text-decoration-skip-ink: none` (сплошная линия, не прерывается выносными), `text-underline-offset: 0.22em` (под нижней границей выносных, не пересекает их). Это editorial-стандарт, не Bootstrap-defaults.
  - **Все hover-правила** живут в одном месте в `global.css` (~строки 134-149). Не разносить по компонентам — иначе теряется обзор «какое поведение где».
...
### 4. ~~Замена картинок в кейсах + починить обрезку~~ ✅ ВЫПОЛНЕНО (2026-05-20, все 4 кейса)
...
- **Mary Trufel ✅ DONE** (2026-05-20, commit `f232945`): 8 PNG ×2 → lossless WebP, paths updated. Существующие `desktopHeight`/`mobileHeight` (964/954/481) уже совпали с реальными размерами экспортов — менять не пришлось.
- **Peptid.ru ✅ DONE** (2026-05-20): 4 PNG ×2 → lossless WebP, paths updated. Старый `main-screen.png` из корня `peptidy/` удалён, новый положен в `figma-sections/` (как в Бюросфере). Высота `2532` в данных совпала с реальным экспортом 5064×2.
- **Все 4 кейса закрыты.** Same pipeline (хранится как памятка для будущих кейсов):
...
### 5. ~~Хедер и футер на мобильном~~ ✅ ЗАКРЫТО (по факту реализовано в сессии 2026-05-14: 2-колонный grid + контакты в строку. Доработано 2026-05-20: gap клампом 10-20px для закона близости на 320-350px, имя в футере → ссылка на главную как в хедере, hover-стили перестроены)
...
### 9. Meta-теги (favicon ✅, OG meta ✅ кроме картинки, title/description final)

**Favicon ✅ закрыт 2026-05-20:**
- `public/favicon.svg` — белые «RZ» Inter Display Bold на чёрном rounded square (20% radius). Глифы зашиты как SVG-paths через fontTools (не зависят от загрузки шрифта в браузере).
- `public/apple-touch-icon.png` 180×180 — для iOS home screen, чёрный фон + белые «RZ».
- Подключены в `BaseLayout.astro` через `<link rel="icon" type="image/svg+xml">` + `<link rel="apple-touch-icon">`.
- Скрипт генерации (для возможной пересборки): был сохранён в `/tmp/make_favicon.py` — использовал fontTools (`pip install fonttools brotli`) для извлечения путей глифов из `public/fonts/InterDisplay-Bold.woff2` и PIL для рендера PNG. Если RZ нужно заменить — пересоздать скрипт или попросить Романа дать готовые ассеты.

**OG/Twitter/canonical meta ✅ закрыты 2026-05-20** (commit `52803a0`):
- В `BaseLayout.astro` добавлены: `<link rel="canonical">`, `og:type/url/title/description/locale`, `twitter:card=summary/title/description`.
- `canonical` и `og:url` вычисляются динамически: `new URL(Astro.url.pathname, Astro.site).href` — каждая страница (/, /projects/burosfera/, etc.) получает свой абсолютный URL.
- `twitter:card=summary` (маленькая карточка). Когда добавим OG-image, поменяем на `summary_large_image`.

**Осталось от пункта 9:**

1. **OG image** — 1200×630 PNG/JPG. Кладётся в `public/og-image.png`, в BaseLayout добавляются 3 строки: `<meta property="og:image" content="https://fantom9000.github.io/og-image.png">` + `og:image:width=1200` + `og:image:height=630`, плюс `twitter:image` + смена `twitter:card` на `summary_large_image`. Картинку нужно от Романа: имя, профессия, на чёрном фоне (как favicon) или светлом — на его вкус. Делается в Figma 5 минут.
2. **Финальные title и description** — сейчас «Роман Жалялов — Дизайнер интерфейсов» / «Портфолио UX/UI-дизайнера Романа Жалялова». Можно переформулировать под SEO (например добавить ключевики «UX/UI-портфолио», «дизайнер интерфейсов из России»). Опционально, текущие тоже валидные.

### 10. ~~Адаптивы кейс-страниц~~ ✅ ПРОВЕРЕНО (2026-05-20, Роман прошёлся по всем 4 кейсам после замены картинок — претензий нет, Pattern-based architecture отработала из коробки)

Историческая заметка (контекст пункта):
